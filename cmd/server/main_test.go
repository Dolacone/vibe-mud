...
func TestLoadConfigUsesFlySQLiteDefaults(t *testing.T) {
	values := map[string]string{"FRONTEND_URL": "https://game.example.test", "GOOGLE_CLIENT_ID": "id", "GOOGLE_CLIENT_SECRET": "secret", "GOOGLE_REDIRECT_URL": "https://api.example.test/auth/google/callback", "COOKIE_SECURE": "false"}
...
func TestLoadConfigKeepsCookiesSecureForInvalidOverride(t *testing.T) {
	values := map[string]string{"FRONTEND_URL": "https://game.example.test", "GOOGLE_CLIENT_ID": "id", "GOOGLE_CLIENT_SECRET": "secret", "GOOGLE_REDIRECT_URL": "https://api.example.test/auth/google/callback", "COOKIE_SECURE": "invalid"}
	cfg, err := loadConfig(func(key string) string { return values[key] })
	if err != nil {
		t.Fatal(err)
	}
