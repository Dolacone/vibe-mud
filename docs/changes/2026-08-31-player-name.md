...
- [x] [Major] REQ-020.10 的 rejected-state 測試只確認 Player name 保持 `Bob`。它未比較已建立的非預設 AP、Inventory、Resource 與 Location。若拒絕路徑清除資產或重設狀態，此測試仍會通過，因此 Task 2 的 rejected state preservation 尚未完成。
