...
- [x] Task 3 沒有對應測試。刪除 `web/functions/proxy.test.ts`、執行既有套件與完成 Docker build，都無法測出 runtime image 漏拷 `web/dist`、路徑錯誤或缺少啟動檔。加入自動化容器整合測試，驗證 image 只含所需 runtime 內容，並能從 image 內的預建檔案提供入口與版本化資源。
