...
- [ ] [Minor] `docs/architecture.md` says Convert accepts only `method_id`, positive `quantity`, and optional `provider_extension_id`. The repaired API also accepts the empty legacy `{}` contract. The documented request contract no longer matches the handler.
