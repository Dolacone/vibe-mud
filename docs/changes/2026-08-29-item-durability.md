...
- [ ] [Major] The uncached full Go suite fails intermittently in `TestBuildSupportsItemOnlyRecipeAndEnforcesLocationUniqueness`. The test compares two complete `PlayerState` values across separate clock reads, so `durability_remaining_seconds` can change at a Unix-second boundary and report a false state mutation.
