...
type ActionKind = "rest" | "move" | "gather" | "convert" | "craft" | "build" | "contribute-construction" | "install-extension" | "repair-building" | "drop" | "pickup" | null;
      ...
      <section aria-labelledby="actions-heading">
        ...
        <TableScroll>
          <table aria-label="Actions">
            ...
            <tbody>
              <tr><th scope="row">Rest</th><td>1 AP</td><td><button type="button" onClick={() => void handleRest()} disabled={actionPendingNow}>{actionPendingNow && actionKind === "rest" ? "Resting..." : "Rest"}</button></td></tr>
            </tbody>
          </table>
        </TableScroll>
      ...
      <section aria-labelledby="building-heading">
        ...
        <TableScroll>
          <table aria-label="Building recipes">
            ...
            <tbody>{(currentUser.building_recipes ?? []).length === 0 ? <EmptyRow colSpan={6}>No building recipes available.</EmptyRow> : (currentUser.building_recipes ?? []).map((recipe) => <tr key={recipe.id}><th scope="row">{recipe.display_name}</th><td>Required AP: {recipe.required_ap}</td><td>Extension slots: {recipe.extension_slot_count}</td><td>{formatInputs(recipe.resource_inputs)}</td><td>{formatInputs(recipe.item_inputs)}</td><td><button type="button" onClick={() => void applyBuildingAction("build", () => build(recipe.id))} disabled={actionPendingNow}>{actionPendingNow && actionKind === "build" ? "Building..." : `Build ${recipe.display_name}`}</button></td></tr>)}</tbody>
          </table>
        </TableScroll>
        <h3>Current location buildings</h3>
        {(currentUser.building_extension_definitions ?? []).length > 0 && <TableScroll><table aria-label="Building extension definitions"><thead><tr><th scope="col">Extension</th><th scope="col">Tier</th><th scope="col">Package item</th><th scope="col">Required AP</th><th scope="col">Controls</th></tr></thead><tbody>{(currentUser.building_extension_definitions ?? []).map((definition) => <tr key={definition.id}><th scope="row">{definition.display_name} T{definition.tier}</th><td>T{definition.tier}</td><td>{definition.package_item.display_name}</td><td>{definition.required_ap}</td><td>{(currentUser.buildings ?? []).filter((building) => building.owner.id === currentUser.id && building.status === "completed" && building.durability_status === "active").flatMap((building) => Array.from({ length: building.extension_slot_count }, (_, slot) => slot).filter((slot) => !(building.extensions ?? []).some((extension) => extension.slot_index === slot)).map((slot) => <button key={`${building.id}-${slot}`} type="button" onClick={() => void handleInstall(building.id, slot, definition.id)} disabled={actionPendingNow}>Install in building {building.id}, slot {slot}</button>))}</td></tr>)}</tbody></table></TableScroll>}
        <TableScroll>
...
function ConversionRow({ method, buildings, disabled, onSubmit }: { method: import("./auth").ConversionMethod; buildings: CurrentUser["buildings"]; disabled: boolean; onSubmit: (quantity: number, providerID?: number) => void }) {
  ...
  const providerRequired = method.id === "sawmill_wood_t1";
  ...
  return <tr><th scope="row">{method.display_name}</th><td>{method.ap_cost} AP</td><td>{method.input.display_name}, max {method.max_input_quantity}</td><td>{method.resource_quantity_per_input} {method.output_resource.display_name}</td><td>{method.essence_item?.display_name ?? "None"}: {method.essence_chance_bps / 100}%</td><td><input aria-label={`Quantity for ${method.display_name}`} type="number" min="1" max={method.max_input_quantity} step="1" value={quantity} onChange={(event) => setQuantity(event.target.value)} disabled={disabled} />{(providers.length > 0 || providerRequired) && <select aria-label={`Provider for ${method.display_name}`} value={provider} onChange={(event) => setProvider(event.target.value)} disabled={disabled}><option value="">Select provider</option>{providers.flatMap((building) => (building.extensions ?? []).filter((extension) => extension.status === "completed" && extension.definition_id === "sawmill_t1").map((extension) => <option key={extension.id} value={extension.id}>{extension.display_name} T{extension.tier}</option>))}</select>}<button type="button" onClick={() => valid && (!providerRequired || provider !== "") && onSubmit(parsed, provider ? Number(provider) : undefined)} disabled={disabled || !valid || (providerRequired && provider === "")}>{disabled ? "Converting..." : "Convert"}</button></td></tr>;
}
